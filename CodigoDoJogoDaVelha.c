...
int jogarJogo(){
   ...
   while(1){
     if(jogada(primeiroJogador)){
        ...
        primeiroJogador = (primeiroJogador == 'X') ? 'O' : 'X'; //Alterna os jogadores     
     }
   }
   menu();
}
...
//strcmp ele compara duas strings
...
void mostrarTabuleiro(){
 ...
 int jogada(char jogador){
    ...
    scanf("%d %d", &linha, &coluna);//pegas os valores dos jogadores
    linha--;//decrementa o valor
    coluna--;//decrementa o valor

    if(linha < 0 || linha > 2 || coluna < 0 || coluna > 2){//verifica
        printf("jogada invalida\n");
        return 0;
    } else if(tabuleiro[linha][coluna] != ' '){//verifica
